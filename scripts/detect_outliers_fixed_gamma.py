...
if __name__ == "__main__":
    ...
    from grandwin.winsorize_detection_fixed_gamma import winsorizing_outlier_detection_3d
    ...
    parser.add_argument("-t", "--data_type", required=True, help="The data type that consist of amplitude, phase, real and imaginary part")
    ...
    parser.add_argument("-st", "--state", type=str, default='both', help="This select the thresholding factor between both, positive and negative")
    ...
    with mp.Pool(processes=mp.cpu_count()) as pool:
        results = pool.starmap(winsorizing_outlier_detection_3d, [(obs_days, grid, obs_list, data_directory, results_directory, integration_time, data_type, final_threshold, gamma, partition, grid_point, state) for obs_days, grid, obs_list in task_list])
